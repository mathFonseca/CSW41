 ...
 *******************************************************************************/
void servo_write0(uint16_t angle){
//	MAP_TimerMatchSet(TIMER2_BASE, TIMER_B, g_ui16perMin*angle/0xFFFF + g_ui16perMin);
	MAP_TimerMatchSet(TIMER2_BASE, TIMER_B, g_ui16perMin*1.75*angle/0xFFFF + (g_ui16perMin/4));
}

void servo_write1(uint16_t angle){
	MAP_TimerMatchSet(TIMER3_BASE, TIMER_B, g_ui16perMin*1.75*angle/0xFFFF + (g_ui16perMin/4));
}

void servo_write2(uint16_t angle){
	MAP_TimerMatchSet(TIMER4_BASE, TIMER_B, g_ui16perMin*angle/0xFFFF + g_ui16perMin);
}

void servo_write3(uint16_t angle){
	MAP_TimerMatchSet(TIMER5_BASE, TIMER_B, g_ui16perMin*angle/0xFFFF + g_ui16perMin);
}
...
servo_init(){
	...
	while(!MAP_SysCtlPeripheralReady(SYSCTL_PERIPH_TIMER2) &
				!MAP_SysCtlPeripheralReady(SYSCTL_PERIPH_TIMER3) &
				!MAP_SysCtlPeripheralReady(SYSCTL_PERIPH_TIMER4) &
				!MAP_SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOB)  &
				!MAP_SysCtlPeripheralReady(SYSCTL_PERIPH_GPIOM));

	// The used ports are PM1, PM3 and PM5, we configure it to use the timer 
	...
	// Configure timers as split pair (A/B) and PWM
	MAP_TimerConfigure(TIMER2_BASE, TIMER_CFG_SPLIT_PAIR | TIMER_CFG_B_PWM);
	MAP_TimerConfigure(TIMER3_BASE, TIMER_CFG_SPLIT_PAIR | TIMER_CFG_B_PWM);
	MAP_TimerConfigure(TIMER4_BASE, TIMER_CFG_SPLIT_PAIR | TIMER_CFG_B_PWM);
	...
	MAP_TimerControlLevel(TIMER5_BASE, TIMER_B, true);
	...
	MAP_TimerPrescaleSet(TIMER5_BASE, TIMER_B, 4);
	...
	MAP_TimerLoadSet(TIMER5_BASE, TIMER_B, g_ui16Period);
	
	
	MAP_TimerMatchSet(TIMER2_BASE, TIMER_B, duty_cycle);
	MAP_TimerMatchSet(TIMER3_BASE, TIMER_B, duty_cycle);
	MAP_TimerMatchSet(TIMER4_BASE, TIMER_B, duty_cycle);
	MAP_TimerMatchSet(TIMER5_BASE, TIMER_B, duty_cycle);
	
	// Starts the PWM timers
